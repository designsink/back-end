...
public class MainPageInfoController {
	...
	@PostMapping
	public ResponseEntity<String> create(@RequestBody MainPageInfoRequestDto requestDto) {
		mainPageInfoService.create(requestDto);
		return ResponseEntity.ok().body("메인페이지 정보 생성 성공");
	}

	@PutMapping("/{infoId}")
	public ResponseEntity<String> update(@PathVariable Integer infoId, @RequestBody MainPageInfoRequestDto requestDto) {
